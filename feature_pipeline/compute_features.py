...
live Hopsworks connection, and it keeps the "what does a feature mean"
...
def add_cyclical_time_features(df: pd.DataFrame, timestamp_col: str = "timestamp") -> pd.DataFrame:
    ...
    # concatenating this row with history read back from Hopsworks
    # (which returns real timestamps, not strings).
...
if __name__ == "__main__":
    ...
    # works, to confirm feature computation before wiring up Hopsworks.
    sample = pd.DataFrame(
        {
